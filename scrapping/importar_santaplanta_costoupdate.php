...
foreach ($data as $row) {
    if ($i == 0) {
        ...
        if ($row[7]) {
            ...
            // Me fijo si el producto ya existe en la BD --> Esto debería hacerse con el SKU, no con el nombre.
...
//ACTUALIZO COSTOS DE SANTA PLANTA EN STOCKMFK [PRECIOS_SANTA_PLANTA=COSTO_MFK]
...
//ACTUALIZO PRECIOS SUGERIDOS DE SANTA PLANTA EN STOCKMFK [PRECIO_SUGERIDO_SANTA_PLANTA=PRECIO_SUGERIDO_MFK]
$query = "UPDATE stockmfk AS s
            JOIN santaplanta AS sp ON s.id = sp.idstock
            SET s.preciosugerido = sp.preciosugerido
            WHERE sp.idstock IS NOT NULL";

if ($mysqli->query($query) === TRUE) {
    echo "Se actualizaron los Precios Sugeridos en STOCKMFK de los productos correspondientes.";
} else {
    echo "Error al actualizar los precios sugeridos: " . $mysqli->error;
}
